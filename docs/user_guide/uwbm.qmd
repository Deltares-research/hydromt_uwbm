...
| Method                 | Explanation                                                                                  |
|------------------------|----------------------------------------------------------------------------------------------|
| `setup_project`        | General model setup, including preparing the region geometry from a region dictionary, and defining timing and CRS. |
| `setup_landuse`        | Generates a landuse map for the region based on provided base files.                         |
| `setup_precip_forcing` | Generates area-averaged, tabular precipitation forcing for the region geometry.              |
| `setup_pet_forcing`    | Generates area-averaged, tabular reference evapotranspiration forcing for the region geometry. |
...
read- and write- methods, i.e. `model.config.read` and `model.config.write` for the `config` component.

| Component | Explanation |
|-----------|-------------|
| `config`  | Model configuration file. |
| `landuse` | Model landuse maps and gridded data. |
...
1. Define the geographical region for which you would like to build a model using the correct HydroMT [region options](https://deltares.github.io/hydromt/latest/user_guide/models/model_region.html).
...
3. Prepare a **model configuration** with the methods that you want to use to add components of your model, see [model configuration](https://deltares.github.io/hydromt/latest/user_guide/models/model_overview.html).
4. **Build** your model using the CLI or Python interface.

```
    hydromt build uwbm path/to/built_model -i uwbm_build.yml -d data_sources.yml -vvv
```
...
You can find more information on the steps to build a model in [HydroMT Core documentation](https://deltares.github.io/hydromt/latest/user_guide/models/model_build.html).
...
2. Prepare or use a pre-defined **data catalog** with all the required data sources, see [working with data](https://deltares.github.io/hydromt/latest/user_guide/data_catalog/data_overview.html).
3. Prepare a **model configuration** with the methods that you want to use to add or change components of your model, see [model configuration](https://deltares.github.io/hydromt/latest/user_guide/models/model_overview.html).
4. **Update** your model using the CLI or Python interface.

```
    hydromt update uwbm path/to/model_to_update -o path/to/updated_model -i uwbm_update.yml -d data_sources.yml -vvv
```
