...
class UWMBConfigWriter:
    def __init__(self, sections: list[ConfigSection] | None = None):
        self.sections: list[ConfigSection] = sections or [ConfigSection()]
        global_section = [s for s in self.sections if s.name is None]
        if not global_section:
            self.sections.insert(0, ConfigSection())

    @staticmethod
    def default() -> "UWMBConfigWriter":
        return UWMBConfigWriter(
            sections=[
                ConfigSection(
                    comments=[
    ...
    def _has_parameter(self, parameter_name: str) -> bool:
        for section in self.sections:
            if parameter_name in section.parameters:
                return True
        return False

    def _get_parameter(self, parameter_name: str) -> tuple[Any | None, str | None]:
        for section in self.sections:
            if parameter_name in section.parameters:
    ...
    def _set_parameter(
        ...
        for section in self.sections:
            if section.name == section_name or (
                section.name is None and section_name is None
            ):
                section.parameters[parameter_name] = value
                return

    def from_dict(self, config_dict: dict[str, Any]) -> None:
        for parameter_name, parameter_value in config_dict.items():
            _, section_name = self._get_parameter(parameter_name)

            self._set_parameter(parameter_name, parameter_value, section_name)
